...
int ajouter_un_contact_dans_rep(Repertoire *rep, Enregistrement enr)
{
	...
	{
		*(rep->tab + rep->nb_elts) = enr; //défini le dernier élément de la liste
		rep->nb_elts++;
		rep->est_trie = false;  // la liste n'est pas à trier
		modif = true;  // on a effectué une modification
		return OK;
	...
	if (rep->nb_elts == 0) {
		...
		int i = 0;
		while (i < rep->nb_elts) && (est_sup(enr, GetElementAt(rep->liste, i)->pers)) { // on augmente un compteur tant qu'on est dans la liste et que enr est superieur
			i++;
		}
		InsertElementAt(rep->list, i, enr);  //on insere donc l elment a l empacement compteur, ainsi deja classe
		rep->nb_elts++;   // on augmente de 1 le nombre d'element
		modif = true;  // on a modifié
	...
	{						/* et que l'indice pointe a l'interieur */
		if (indice < rep->nb_elts) { //si l'indice est dans le tableau
			for (int i = indice; i < rep->nb_elts; i++) {
				*(rep->tab + i) = *(rep->tab + i + 1); // on décale chaque élément de sorte à supprimer l'élément d'indice i
			}
			rep->nb_elts -= 1;
			modif = true;
		}
...
{
	printf("\n%s,  %s,  %s", enr.nom, enr.prenom, enr.tel); // pour afficher le nom, prénom et numéro
...
{
	if (_stricmp(enr1.nom, enr2.nom) > 0) { return true; } // on compare d'abord les noms, on retourne vrai si le premier est plus grand que le deuxieme
	if (_stricmp(enr1.nom, enr2.prenom) < 0) // on retourne faux sinon
	{
		return (false);
	}
	if (_stricmp(enr1.nom, enr2.nom) == 0) { // si les noms sont égaux on recommence avec les prénoms
...
void trier(Repertoire *rep)
{
	...
	while (flag != 0) {  // on continue jusqu'à ce que le tableau soit trié
		flag = 0; // on remet le flag à zéro
		for (int i = 0; i < rep->nb_elts; i++) // on parcourt tous les éléments jusqu'a l'avant dernier
		{
			if (est_sup(rep->tab[i], rep->tab[i + 1])) { // si un element est supérieur à celui d'après
				Enregistrement a = rep->tab[i];
...
int rechercher_nom(Repertoire* rep, char nom[], int ind)
{
	...
	ind_fin = rep->nb_elts;
	for (int i = ind; i < ind_fin; i++) { // on utilise la même manière que la liste à l'exeption de la manière dont on recupère les élément
		strcpy_s(tmp_nom2, MAX_NOM, GetElementAt(rep->list, i)->pers.nom);
...
int sauvegarder(Repertoire *rep, char nom_fichier[])
{
	...
	}//une fois la ligne mise dans buffer on vient l'écrire dans le fichier
#else
#ifdef IMPL_LIST
